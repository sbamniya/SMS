...
socialApp.controller('ViewVendorForManager', ['$scope', '$http', '$routeParams', '$route', '$timeout', function($scope, $http, $routeParams, $route, $timeout) {
    ...
    $scope.updateId = function(vendor_id, vendor_name, email, contact, payuavailable, merchant_id, merchant_key, merchant_salt, description) {
        if (payuavailable == 1 && merchant_id != "") {
            $scope.payuavailable == 1;
        }
        $scope.updateData = {
            id: vendor_id,
