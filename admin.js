...
exports.serviceadminLogin = function(crypto, pool) {
    return function(req, res) {
        {
            sess = req.session;
            //console.log("DB connection established");
            var userName = req.body.userName;
            var password = req.body.password;
            var queryString = 'SELECT * FROM serviceadmin where username = "' + userName + '"';
            var result = {};
            pool.query(queryString, function(err, rows, fields) {
                ...
                if (err) {
                    ...
                    if (rows.length == 0) {
                        ...
                        if (rows[0].status == 1) { //Creating hash with received password value for comparison : DR
                            var passwordn = 12345;//crypto.createHash('md5').update(password).digest("hex");
                            if (passwordn == rows[0].password) {
                                sess.userID = rows[0].id;
                                sess.userPrivilege = 1;
